...
def greenplm_create(
    ...
    # Chinese test
    print("Chinese Tokenization TEST:")
    for word in ["hello", "machine learning", "artificial intelligence", "deep learning"]:
...
def greenplm_create_st(
    ...
    # Test
    print("Chinese test:")
    for word in ["hello", "machine learning", "artificial intelligence"]:
        ids = tokenizer.encode(word, add_special_tokens=False)
        decoded = tokenizer.decode(ids).replace(' ', '')
        status = "✓" if decoded == word else "✗"
        print(f"  {status} '{word}' → {ids}")
    ...
    # Embedding test
    print("Chinese Embedding test:")
    test_zh = ["hello world", "machine learning is interesting"]
